...
/*   Updated: 2026/05/28 14:26:50 by eprieur          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	load_next_texture(t_graphics *graph)
{
	...
	graph->pars->tex.east_wall.img_addr = mlx_get_data_addr(
			graph->pars->tex.east_wall.img,
			&graph->pars->tex.east_wall.bits_pp,
			&graph->pars->tex.east_wall.size_line,
			&graph->pars->tex.east_wall.endian);
	graph->pars->tex.west_wall.img = mlx_xpm_file_to_image(
			graph->mlx,
			graph->pars->config.west_texture, &w, &h);
	graph->pars->tex.west_wall.img_addr = mlx_get_data_addr(
			graph->pars->tex.west_wall.img,
...
void	load_texture(t_graphics *graph)
{
	...
	graph->pars->tex.north_wall.img_addr = mlx_get_data_addr(
			graph->pars->tex.north_wall.img,
			&graph->pars->tex.north_wall.bits_pp,
			&graph->pars->tex.north_wall.size_line,
			&graph->pars->tex.north_wall.endian);
	graph->pars->tex.south_wall.img = mlx_xpm_file_to_image(
			graph->mlx,
			graph->pars->config.south_texture, &w, &h);
	graph->pars->tex.south_wall.img_addr = mlx_get_data_addr(
			graph->pars->tex.south_wall.img,
